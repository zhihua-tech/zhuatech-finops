...
package cn.zhuatech.finops.config;
import cn.zhuatech.finops.domain.DomainCatalog;
import cn.zhuatech.finops.model.*;
import cn.zhuatech.finops.repository.*;
...
@Configuration public class DataInitializer {
 ...
 @Bean CommandLineRunner seed(BusinessRecordRepository records,SystemSettingRepository settings,DomainCatalog catalog) {
  return args -> { if(records.count()>0)return;
            ...
            settings.save(new SystemSetting("recordNoRule", "FINOPS-{YYYY}-{SEQ}"));
   ...
   int sequence=1; for(var module:catalog.modules()) {
    String no="FINOPS-DEMO-"+String.format("%03d",sequence);
